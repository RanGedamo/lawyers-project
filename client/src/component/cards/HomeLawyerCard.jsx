...
import {  Card, CardContent, CardMedia, Typography, CardActionArea, Avatar, Stack, Divider, Button, Box, List, ListItem } from "@mui/material"
...
export default function HomeLawyerCard() {
  const Lawyers = [
    {
      id: "1",
  ...
  const Lawyer = {
    ...
    intredaction: "im a problem solver",
    servicesSubCategories: ["a", "b", "c", "d"],
    location: "Belgim",
    expiriance: "3 years",
    avgResplayTime: 3.4,
    workDueTime: "10 days",
  ...
  return (
    <Card sx={{ maxWidth: 345 }}>
      <CardActionArea>
        ...
        <Typography>{Lawyer.filedCategory}</Typography>
        <PersonalRating rate={Lawyer.rate} />
        <CardContent>
          <Typography variant="body2" color="text.secondary">
            {Lawyer.intredaction}
            <Divider variant="middle" />
            {/* {Lawyer.reviwes?.map((reviwe, i) => (
              <div key={i}>
                <Typography>{reviwe.comment}</Typography>
                <Typography>{reviwe.user}</Typography>
              </div>
            ))} */}
          </Typography>

          <Typography>{Lawyer.intredaction}</Typography>
          <List>
        ...
        <Box sx={{ mt: 3, ml: 1, mb: 1 }}>
          <Stack spacing={1} direction="row">
            ...
            <Button variant="outlined">Get an offer</Button>
          </Stack>
        </Box>
      </CardActionArea>
