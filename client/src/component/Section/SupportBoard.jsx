...
export default function SupportBoard() {
  ...
  const [errorSendDomain, setErrorSendDomain] = useState(false)
  ...
  const submitDomain = async() => {
    console.log(domain);
    await postDomain(domain).then(res=>console.log(res.response.data.message))
    .catch(err=>{
     if(err.response.data.message){
      setTimeout(() => {
        setErrorSendDomain(false)
      }, 5000)
      return setErrorSendDomain(false);
     }
